...
class SaveSimilarityOptions extends Component {
    ...
    submit = async  () => {
        let rules = this.props.form.rules
        let fileA = this.props.form.files[0]
        let fileB = this.props.form.files[1]
        const promise = await axios.post('http://localhost:8000/link_similarity/'+fileA+'/'+fileB+"/"+this.state.name+"/"+this.state.description, {"rules": rules})
...
export default SaveSimilarityOptions;